...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Videogames');
}
...
async function listGames(
  ...
  if (!res.ok) throw await httpError(res, 'FreeToGame API error');
...
async function getGame(id: number) {
  const res = await pwFetch(`${BASE_URL}/game?id=${id}`);
...
async function filterGames(tag: string, platform?: string) {
  ...
  const res = await pwFetch(`${BASE_URL}/filter?${params}`);
  if (!res.ok) throw await httpError(res, 'FreeToGame API error');
