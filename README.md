...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/list_games \
  -H 'Content-Type: application/json' \
  -d '{"platform":"pc","category":"shooter","sort_by":"popularity"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/list_games`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "videogames": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-videogames"]
    }
  }
}
```
